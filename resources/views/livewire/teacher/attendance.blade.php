    ...
    <div class="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-100 dark:border-zinc-800 shadow-xs p-5">
        <div class="flex flex-col md:flex-row gap-4 items-end">
            ...
            <div class="w-full md:w-48">
                <flux:input type="date" wire:model.live="date" label="التاريخ" />
            </div>

            <div class="flex gap-1 p-1 bg-zinc-100 dark:bg-zinc-800 rounded-lg h-fit">
                <button
                    wire:click="switchMode('wizard')"
            ...
            @if($students->count() > 0 && !$isComplete)
                <flux:button wire:click="markAllPresent" variant="ghost" size="sm" class="text-green-600 hover:text-green-700 dark:text-green-400">
        ...
        @if($students->count() > 0)
            <div class="mt-4">
                ...
                <div class="w-full bg-zinc-200 dark:bg-zinc-700 rounded-full h-2 overflow-hidden">
                    <div
                        class="h-2 rounded-full transition-all duration-500 ease-out {{ $isComplete ? 'bg-green-500' : 'bg-blue-500' }}"
        ...
        <div class="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-100 dark:border-zinc-800 shadow-xs overflow-hidden">
            @if($isComplete)
                ...
                <div class="p-16 text-center space-y-4">
                    ...
                    <div class="pt-4">
                        <flux:button wire:click="switchMode('list')" variant="primary" class="bg-zinc-800 hover:bg-zinc-700 dark:bg-zinc-200 dark:text-zinc-800 dark:hover:bg-zinc-300">
                    ...
                    <div class="p-8 md:p-12 text-center space-y-8">
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="pt-4">
                                ...
                                <h2 class="text-3xl md:text-4xl font-bold text-zinc-900 dark:text-white">{{ $currentStudent->name }}</h2>
                                @if($currentStudent->circle)
                                    <p class="text-sm text-zinc-400 dark:text-zinc-500 mt-1">{{ $currentStudent->circle->name }}</p>
                                @endif
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 max-w-2xl mx-auto">
                            <button
                                wire:click="markStatus({{ $currentStudent->id }}, 'present')"
                                class="group flex flex-col items-center gap-3 p-6 rounded-2xl border-2 transition-all duration-200
                            ...
                            >
                                ...
                                <span class="font-semibold text-green-700 dark:text-green-400">حاضر</span>
                            </button>

                            <button
                                wire:click="markStatus({{ $currentStudent->id }}, 'absent')"
                                class="group flex flex-col items-center gap-3 p-6 rounded-2xl border-2 transition-all duration-200
                            ...
                            >
                                <span class="text-3xl">❌</span>
                                <span class="font-semibold text-red-700 dark:text-red-400">غائب</span>
                            </button>

                            <button
                                wire:click="markStatus({{ $currentStudent->id }}, 'late')"
                                class="group flex flex-col items-center gap-3 p-6 rounded-2xl border-2 transition-all duration-200
                            ...
                            >
                                <span class="text-3xl">⏰</span>
                                <span class="font-semibold text-amber-700 dark:text-amber-400">متأخر</span>
                            </button>

                            <button
                                wire:click="markStatus({{ $currentStudent->id }}, 'excused')"
                                class="group flex flex-col items-center gap-3 p-6 rounded-2xl border-2 transition-all duration-200
                            ...
                            >
                                <span class="text-3xl">🔔</span>
                                <span class="font-semibold text-blue-700 dark:text-blue-400">مستأذن</span>
                            </button>
                        </div>
                        ...
                        <div class="flex items-center justify-center gap-4 pt-4">
                            ...
                            <div class="flex items-center gap-1 max-w-xs overflow-hidden">
                                @foreach($students as $idx => $s)
                                    <div class="size-2 rounded-full transition-all {{ $idx === $currentIndex ? 'bg-blue-500 scale-125' : (!empty($records[$s->id]) ? 'bg-green-400' : 'bg-zinc-300 dark:bg-zinc-600') }}"></div>
                                @endforeach
                            </div>
                            ...
                            <flux:button
                                ...
                                :disabled="$currentIndex === count($students) - 1"
                            >
                                التالي
                            </flux:button>
                        </div>
        ...
        <div class="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-100 dark:border-zinc-800 shadow-xs overflow-hidden">
            <div class="divide-y divide-zinc-100 dark:divide-zinc-800">
                @foreach($students as $index => $student)
                    <div wire:key="student-{{ $student->id }}" class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 p-4 hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors">
                        <div class="flex items-center gap-3">
