@php
    $statusLabels = ['pending' => 'بانتظار الرسوم', 'confirmed' => 'مؤكَّد', 'received' => 'استُلم', 'cancelled' => 'ملغى'];
    $statusColors = ['pending' => 'amber', 'confirmed' => 'emerald', 'received' => 'zinc', 'cancelled' => 'red'];
    $standingLabels = ['ok' => 'عادية', 'prepay' => 'رسوم مقدماً', 'banned' => 'محرومة'];
    $standingColors = ['ok' => 'emerald', 'prepay' => 'amber', 'banned' => 'red'];
@endphp

<div class="min-h-screen bg-zinc-50 dark:bg-zinc-950 py-6 px-4 sm:py-8">
    <div class="max-w-3xl mx-auto space-y-5">

        <div class="text-center">
            <flux:heading size="xl" class="font-bold">مسؤول الباصات</flux:heading>
            <flux:subheading>الاستلام، والرسوم، وحالة المراحل</flux:subheading>
        </div>

        {{-- ─────────── المراحل وحالاتها ─────────── --}}
        <div class="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-100 dark:border-zinc-800 shadow-xs p-4 sm:p-5">
            <flux:heading size="lg" class="mb-3">المراحل</flux:heading>

            {{-- A badge only where there is something to say. Most stages are in the
                 clear, and five "عادية" chips crowd a phone while hiding the one
                 stage the officer actually needs to see. --}}
            <div class="flex flex-wrap gap-2">
                @foreach ($stages as $stage)
                    @php $clear = ($stage->bus_standing ?? 'ok') === 'ok'; @endphp
                    <button type="button" wire:key="stage-{{ $stage->id }}" wire:click="openRuling({{ $stage->id }})"
                        @class([
                            'inline-flex items-center gap-2 px-3 h-9 rounded-lg border text-sm transition',
                            'border-zinc-200 dark:border-zinc-700 hover:bg-zinc-50 dark:hover:bg-zinc-800' => $clear,
                            'border-amber-300 bg-amber-50 dark:border-amber-900/60 dark:bg-amber-950/30' => ($stage->bus_standing ?? '') === 'prepay',
                            'border-red-300 bg-red-50 dark:border-red-900/60 dark:bg-red-950/30' => ($stage->bus_standing ?? '') === 'banned',
                        ])>
                        <span @class([
                            'size-2 rounded-full shrink-0',
                            'bg-emerald-500' => $clear,
                            'bg-amber-500' => ($stage->bus_standing ?? '') === 'prepay',
                            'bg-red-500' => ($stage->bus_standing ?? '') === 'banned',
                        ])></span>
                        <span class="text-zinc-700 dark:text-zinc-200 whitespace-nowrap">{{ $stage->name }}</span>
                        @unless ($clear)
                            <span class="text-xs whitespace-nowrap {{ ($stage->bus_standing ?? '') === 'banned' ? 'text-red-600 dark:text-red-400' : 'text-amber-700 dark:text-amber-400' }}">
                                {{ $standingLabels[$stage->bus_standing] ?? '' }}
                            </span>
                        @endunless
                    </button>
                @endforeach
            </div>

            {{-- لوحة القرار، سواء تبع استلاماً أو بدأه المسؤول من هنا --}}
            @if ($rulingStage)
                <div class="mt-4 rounded-xl border border-maroon/40 dark:border-white/20 p-4 space-y-3">
                    <div class="text-sm font-bold text-zinc-800 dark:text-zinc-100">
                        قرار بشأن «{{ $rulingStage->name }}»
                    </div>

                    <flux:select wire:model="rulingStanding" size="sm" label="الحالة الجديدة">
                        <flux:select.option value="ok">عادية — تحجز بلا قيد</flux:select.option>
                        <flux:select.option value="prepay">رسوم مقدماً — يُسجَّل الحجز ولا يُؤكَّد حتى تُدفع</flux:select.option>
                        <flux:select.option value="banned">محرومة — لا تحجز إطلاقاً</flux:select.option>
                    </flux:select>

                    <flux:input wire:model="rulingReason" size="sm" label="السبب"
                        placeholder="يظهر للمرحلة عند محاولة الحجز" />

                    <div class="flex flex-col-reverse sm:flex-row sm:justify-end gap-2">
                        <flux:button size="sm" variant="ghost" class="w-full sm:w-auto" wire:click="closePanels">إلغاء</flux:button>
                        @if ($rulingBookingId)
                            <flux:button size="sm" variant="ghost" class="w-full sm:w-auto" wire:click="overlook">تجاوز هذه المرة</flux:button>
                        @endif
                        <flux:button size="sm" variant="primary" class="w-full sm:w-auto !h-11" wire:click="saveRuling">حفظ القرار</flux:button>
                    </div>
                </div>
            @endif
        </div>

        {{-- ─────────── الحجوزات ─────────── --}}
        <div class="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-100 dark:border-zinc-800 shadow-xs overflow-hidden">
            {{-- Short labels, and the count as a pill of its own: spelled out, the
                 first tab wrapped onto three lines and left the bar lopsided. --}}
            <div class="flex items-center gap-1 p-1 m-4 bg-zinc-100 dark:bg-zinc-800 rounded-xl">
                @foreach (['waiting' => 'للاستلام', 'upcoming' => 'القادمة', 'all' => 'الكل'] as $key => $label)
                    <button type="button" wire:key="filter-{{ $key }}" wire:click="setFilter('{{ $key }}')"
                        @class([
                            'flex-1 inline-flex items-center justify-center gap-1.5 h-10 px-2 text-sm font-medium rounded-lg whitespace-nowrap',
                            'bg-white dark:bg-zinc-700 text-zinc-900 dark:text-white shadow-sm' => $filter === $key,
                            'text-zinc-500 dark:text-zinc-400' => $filter !== $key,
                        ])>
                        {{ $label }}
                        @if ($key === 'waiting' && $waitingCount > 0)
                            <span class="inline-flex items-center justify-center min-w-5 h-5 px-1 rounded-full bg-maroon text-white text-[11px] font-bold">
                                {{ App\Support\HijriDate::arabicDigits($waitingCount) }}
                            </span>
                        @endif
                    </button>
                @endforeach
            </div>

            <div class="divide-y divide-zinc-100 dark:divide-zinc-800">
                @forelse ($bookings as $booking)
                    <div wire:key="booking-{{ $booking->id }}" class="px-4 sm:px-5 py-4">
                        {{-- The name and the state read as one line; the actions get a
                             row of their own underneath. Stacked in a column beside the
                             text they left a ragged edge and wasted a phone's height. --}}
                        <div class="flex items-start justify-between gap-3">
                            <div class="min-w-0">
                                <div class="text-base font-bold text-zinc-800 dark:text-zinc-100 truncate">
                                    {{ $booking->stage?->name ?? '—' }}
                                </div>
                                <div class="text-xs text-zinc-400 mt-0.5">
                                    {{ $this->hijri($booking->date->format('Y-m-d')) }}
                                </div>
                                <div class="text-sm text-zinc-600 dark:text-zinc-300 mt-1 truncate">
                                    {{ $booking->buses->pluck('name')->implode('، ') }}
                                </div>
                                @if ($booking->isPending() && $booking->fee_due_on)
                                    <div class="text-xs mt-1 {{ $booking->hasLapsed() ? 'text-red-600 dark:text-red-400' : 'text-amber-600 dark:text-amber-400' }}">
                                        @if ($booking->hasLapsed())
                                            انقضى موعد الدفع — لم يعد يحجز الباصات
                                        @else
                                            موعد الدفع: {{ App\Support\HijriDate::weekday($booking->fee_due_on) }}
                                            {{ App\Support\HijriDate::dayMonth($booking->fee_due_on) }}
                                        @endif
                                    </div>
                                @endif
                            </div>

                            <flux:badge size="sm" :color="$statusColors[$booking->status] ?? 'zinc'" class="shrink-0">
                                {{ $statusLabels[$booking->status] ?? $booking->status }}
                            </flux:badge>
                        </div>

                        @if (! $booking->isCancelled() && $receivingId !== $booking->id)
                            <div class="flex flex-wrap items-center gap-2 mt-3">
                                @if ($booking->isPending())
                                    <flux:button size="sm" variant="primary" class="flex-1 sm:flex-none !h-10"
                                        wire:click="markPaid({{ $booking->id }})">
                                        استلمت {{ App\Support\HijriDate::arabicDigits($booking->fee_total) }} ﷼
                                    </flux:button>
                                @endif

                                @if ($this->canReceive($booking))
                                    <flux:button size="sm" variant="filled" icon="inbox-arrow-down"
                                        class="flex-1 sm:flex-none !h-10" wire:click="startReceive({{ $booking->id }})">
                                        استلام
                                    </flux:button>
                                @endif

                                <flux:button size="sm" variant="ghost" class="text-red-400 hover:text-red-600 !h-10"
                                    wire:click="cancel({{ $booking->id }})"
                                    wire:confirm="إلغاء حجز «{{ $booking->stage?->name }}»؟">إلغاء</flux:button>
                            </div>
                        @endif

                        {{-- الخطوة الثانية: تأشير البنود --}}
                        @if ($receivingId === $booking->id)
                            <div class="mt-4 rounded-xl border border-maroon/40 dark:border-white/20 p-4 space-y-3">
                                <div class="text-sm font-bold text-zinc-800 dark:text-zinc-100">
                                    أشّر ما أُنجز عند التسليم
                                </div>

                                @forelse ($items as $item)
                                    <label wire:key="check-{{ $item->id }}"
                                        class="flex items-center gap-3 p-3.5 rounded-lg border border-zinc-200 dark:border-zinc-700 cursor-pointer hover:bg-zinc-50 dark:hover:bg-zinc-800/50 has-[:checked]:border-emerald-400 has-[:checked]:bg-emerald-50 dark:has-[:checked]:bg-emerald-950/20">
                                        <input type="checkbox" wire:model="doneItems" value="{{ $item->id }}"
                                            class="size-5 rounded border-zinc-300 text-emerald-600 focus:ring-emerald-500">
                                        <span class="text-sm text-zinc-700 dark:text-zinc-200">{{ $item->label }}</span>
                                    </label>
                                @empty
                                    <div class="text-sm text-zinc-400">لا بنود معرَّفة. أضفها من لوحة المدير.</div>
                                @endforelse

                                <div class="flex flex-col-reverse sm:flex-row sm:justify-end gap-2">
                                    <flux:button size="sm" variant="ghost" class="w-full sm:w-auto" wire:click="closePanels">إلغاء</flux:button>
                                    <flux:button size="sm" variant="primary" class="w-full sm:w-auto !h-11" wire:click="saveReceive">تسجيل الاستلام</flux:button>
                                </div>
                            </div>
                        @endif

                        {{-- ما سُجّل عند استلام سابق --}}
                        @if ($booking->checks->isNotEmpty() && $receivingId !== $booking->id)
                            <div class="mt-3 flex flex-wrap gap-1.5">
                                @foreach ($booking->checks as $check)
                                    <span wire:key="done-{{ $check->id }}"
                                        @class([
                                            'inline-flex items-center gap-1 px-2 py-0.5 rounded text-[11px]',
                                            'bg-emerald-50 text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-300' => $check->is_done,
                                            'bg-red-50 text-red-700 dark:bg-red-950/40 dark:text-red-300' => ! $check->is_done,
                                        ])>
                                        <flux:icon :icon="$check->is_done ? 'check' : 'x-mark'" class="size-3" />
                                        {{ $check->label }}
                                    </span>
                                @endforeach
                            </div>
                        @endif
                    </div>
                @empty
                    <div class="px-5 py-16 text-center">
                        <flux:icon icon="truck" class="size-12 mx-auto text-zinc-300 dark:text-zinc-600 mb-3" />
                        <div class="text-sm text-zinc-400">
                            @if ($filter === 'waiting') لا شيء بانتظار الاستلام. @else لا حجوزات. @endif
                        </div>
                    </div>
                @endforelse
            </div>
        </div>
    </div>
</div>
